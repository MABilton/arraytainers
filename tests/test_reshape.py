...
reshape_tests_1 = [
    ...
    },
    {
        ...
        "order": "F",
        "expected": {
            "a": {"c": [np.array([[1, 5, 4], [3, 2, 6]])]},
            "b": [{"a": np.array([[6, 4, 2], [5, 3, 1]])}],
        },
    },
]
...
@pytest.mark.parametrize(
    "contents, shape, order, expected",
    [
        (test["contents"], test["shape"], test["order"], test["expected"])
        for test in reshape_tests_1
    ],
)
def test_arraytainer_reshape_with_tuple_shape_and_string_order(
    contents, shape, order, expected
):
...
def test_jaxtainer_reshape_with_tuple_shape_and_string_order(
    contents, shape, order, expected
):
...
@pytest.mark.parametrize(
    ...
    [
        (test["contents"], test["shape"], test["order"], test["expected"])
        for test in reshape_tests_2
    ],
)
def test_arraytainer_reshape_with_arraytainer_shape_and_string_order(
    contents, shape, order, expected
):
    contents = helpers.deepcopy_contents(contents)
    expected = helpers.deepcopy_contents(expected)
    ...
    arraytainer = Arraytainer(contents)
    shape_w_arrayconversion = [
        Arraytainer(val, dtype=int) if not isinstance(val, int) else val
        for val in shape
    ]
    shape_wo_arrayconversion = [
        Arraytainer(val, array_conversions=False, dtype=int)
...
def test_jaxtainer_reshape_with_jaxtainer_shape_and_string_order(
    contents, shape, order, expected
):
    contents = helpers.deepcopy_contents(contents, has_jax_arrays=True)
    expected = helpers.deepcopy_contents(expected, has_jax_arrays=True)
    ...
    jaxtainer = Jaxtainer(contents)
    shape_w_arrayconversion = [
        Jaxtainer(val, dtype=int) if not isinstance(val, int) else val for val in shape
    ]
    shape_wo_arrayconversion = [
        Jaxtainer(val, array_conversions=False, dtype=int)
        if not isinstance(val, int)
        else val
        for val in shape
    ]
...
@pytest.mark.parametrize(
    ...
    [
        (test["contents"], test["shape"], test["order"], test["expected"])
        for test in reshape_tests_3
    ],
)
def test_arraytainer_reshape_with_tuple_shape_and_arraytainer_order(
    contents, shape, order, expected
):
    contents = helpers.deepcopy_contents(contents)
    expected = helpers.deepcopy_contents(expected)
...
def test_jaxtainer_reshape_with_tuple_shape_and_jaxtainer_order(
    contents, shape, order, expected
):
    contents = helpers.deepcopy_contents(contents, has_jax_arrays=True)
    expected = helpers.deepcopy_contents(expected, has_jax_arrays=True)
...
@pytest.mark.parametrize(
    ...
    [
        (test["contents"], test["shape"], test["order"], test["expected"])
        for test in reshape_tests_4
    ],
)
def test_arraytainer_reshape_with_arraytainer_shape_and_arraytainer_order(
    contents, shape, order, expected
):
    contents = helpers.deepcopy_contents(contents)
    expected = helpers.deepcopy_contents(expected)
...
def test_jaxtainer_reshape_with_jaxtainer_shape_and_jaxtainer_order(
    contents, shape, order, expected
):
    contents = helpers.deepcopy_contents(contents, has_jax_arrays=True)
    expected = helpers.deepcopy_contents(expected, has_jax_arrays=True)
