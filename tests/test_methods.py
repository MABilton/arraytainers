...
@pytest.mark.parametrize("contents, key_iterable, new_val, expected", [(test['contents'], test['key_iterable'], test['new_val'], test['expected']) for test in append_list_tests_vals])
def test_arraytainer_append_with_nonarraytainer(contents, key_iterable, new_val, expected):
    contents = helpers.deepcopy_contents(contents)
    expected = helpers.deepcopy_contents(expected)
    new_val = helpers.deepcopy_contents(new_val)
...
@pytest.mark.parametrize("contents, key_iterable, new_val, expected", [(test['contents'], test['key_iterable'], test['new_val'], test['expected']) for test in append_list_tests_arraytainers])
...
@pytest.mark.parametrize("contents, key_iterable", [(test['contents'], test['key_iterable']) for test in too_many_keys_tests])
def test_arraytainer_update_error_too_many_keys_provided(contents, key_iterable):
    new_val = {'a': np.array(1)}
    contents = helpers.deepcopy_contents(contents)
    arraytainer = Arraytainer(contents)

    with pytest.raises(KeyError, match="(not an Arraytainer)|(not a key in this Arraytainer)"):
        arraytainer.update(new_val, key_iterable)
...
@pytest.mark.parametrize("contents, expected", [(test['contents'], test['expected']) for test in tolist_tests])
