...
transpose_tests = [
    ...
    {
        "contents": {
            "a": [np.array([[1, 2], [3, 4]]), np.array([[9], [10]])],
            "b": {"c": [np.array([[5, 6], [7, 8]]), np.array([[[9, 10]]])]},
        },
        "expected": {
            "a": [np.array([[1, 2], [3, 4]]).T, np.array([[9], [10]]).T],
            "b": {"c": [np.array([[5, 6], [7, 8]]).T, np.array([[[9, 10]]]).T]},
        },
    },
]
...
@pytest.mark.parametrize(
    "contents, expected",
    [(test["contents"], test["expected"]) for test in transpose_tests],
)
...
size_tests = [
    {
        "contents": {"a": np.ones((2, 2)), "b": np.ones((3, 3, 3)), "c": 2},
        "expected": 2**2 + 3**3 + 1,
    },
    {
        "contents": [np.ones((2, 2)), np.ones((3, 3, 3)), 2],
        "expected": 2**2 + 3**3 + 1,
    },
    {
        "contents": {
            "a": [np.ones((3, 3, 3)), np.ones((2, 2))],
            "b": {"c": [np.ones((2, 2, 2)), 1]},
        },
        "expected": 3**3 + 2**2 + 2**3 + 1,
    },
]


@pytest.mark.parametrize(
    "contents, expected", [(test["contents"], test["expected"]) for test in size_tests]
)
...
flatten_tests = [
    ...
    # Nested arraytainers:
    {
        "contents": {
            ...
            1: [{"a": np.array([[11, 12], [13, 14]])}, np.array(15)],
        },
        "order": None,
...
@pytest.mark.parametrize(
    "contents, order, expected",
    [(test["contents"], test["order"], test["expected"]) for test in flatten_tests],
)
...
any_tests = [
    {
        "contents": {
            "a": [{"c": False}, np.zeros((2, 2))],
            "b": {"d": [np.array([False]), 0]},
        },
        "expected": False,
    },
    {
        "contents": {
            "a": [{"c": True}, np.zeros((2, 2))],
            "b": {"d": [np.array([False]), 0]},
        },
        "expected": True,
    },
...
@pytest.mark.parametrize(
    "contents, expected", [(test["contents"], test["expected"]) for test in any_tests]
)
...
@pytest.mark.parametrize(
    "contents, expected", [(test["contents"], test["expected"]) for test in all_tests]
)
...
list_arrays_arraytainer_tests = [
    ...
    {
        "contents": {
            "a": {"c": [np.ones((3, 3, 3)), np.array([[9, 10]])]},
            1: [{"a": np.array([[11, 12], [13, 14]])}, np.array(15)],
        },
        "expected": [
            np.ones((3, 3, 3)),
...
@pytest.mark.parametrize(
    "contents, expected",
    [(test["contents"], test["expected"]) for test in list_arrays_arraytainer_tests],
)
...
list_arrays_jaxtainer_tests = [
    ...
    {
        "contents": {
            "a": {"c": [jnp.ones((3, 3, 3)), jnp.array([[9, 10]])]},
            1: [{"a": jnp.array([[11, 12], [13, 14]])}, jnp.array(15)],
        },
        "expected": [
            jnp.ones((3, 3, 3)),
            jnp.array([[9, 10]]),
            jnp.array([[11, 12], [13, 14]]),
            jnp.array(15),
        ],
    },
    {
        "contents": {
            "a": [{"c": True}, jnp.ones((2, 2))],
            "b": {"d": [jnp.array([True]), 1]},
        },
        "expected": [
            jnp.array(True),
            jnp.ones((2, 2)),
...
@pytest.mark.parametrize(
    "contents, expected",
    [(test["contents"], test["expected"]) for test in list_arrays_jaxtainer_tests],
)
...
@pytest.mark.parametrize(
    "contents, expected", [(test["contents"], test["expected"]) for test in sum_tests]
)
...
@pytest.mark.parametrize(
    "contents, expected",
    [(test["contents"], test["expected"]) for test in sum_arrays_tests],
)
...
sum_elems_tests = [
    ...
    {
        "contents": {
            "a": {"a": np.array([1, 2]), "b": np.array([[3, 4], [5, 6]])},
            "b": {"a": 1, "b": 2},
        },
    ...
    {
        "contents": {
            "a": [{"a": np.array([[1, 2], [3, 4]])}, {"b": np.array([[5, 6], [7, 8]])}],
...
@pytest.mark.parametrize(
    "contents, expected",
    [(test["contents"], test["expected"]) for test in sum_elems_tests],
)
