...
@pytest.mark.parametrize(
    "contents, function, expected",
    [(test["contents"], test["function"], test["expected"]) for test in vmap_tests],
)
...
jacfwd_tests = [
    ...
    },
    {
        ...
        "expected": {
            "a": [{"c": jnp.array([[2, 0], [0, 4]])}],
            "0": jnp.array([[1 / 2, 0], [0, 1 / 3]]),
        },
    },
    {
        "contents": {"a": [{"c": jnp.array([1, 2])}], "0": jnp.array([2, 3])},
...
@pytest.mark.parametrize(
    "contents, function, expected",
    [(test["contents"], test["function"], test["expected"]) for test in jacfwd_tests],
)
...
jacfwd_and_vmap_tests = [
    ...
    # Func returns array:
    {
        ...
        "function": lambda x: x["a"] ** 2 + jnp.log(x["0"]),
        "expected": {
            "a": jnp.array([[[2.0, 0.0], [0.0, 4.0]], [[6.0, 0.0], [0.0, 8.0]]]),
            "0": jnp.array([[1 / 2, 1 / 2], [1 / 3, 1 / 3]]),
        },
    },
    {
        "contents": [jnp.array([[1, 2], [3, 4]]), jnp.array([2, 3])],
        ...
        "expected": [
            jnp.array([[[2, 0], [0, 4]], [[6, 0], [0, 8]]]),
            jnp.array([[1 / 2, 1 / 2], [1 / 3, 1 / 3]]),
        ],
    },
    {
        "contents": {"a": [{"c": jnp.array([[1, 2], [3, 4]])}], "0": jnp.array([2, 3])},
...
@pytest.mark.parametrize(
    "contents, function, expected",
    [
        (test["contents"], test["function"], test["expected"])
        for test in jacfwd_and_vmap_tests
    ],
)
...
jacfwd_and_vmap_constructor_tests = [
    {
        ...
        "expected": {
            "a": jnp.array([[2, 0, 0], [4, 0, 0]]),
            "b": jnp.array([[0, 4, 0], [0, 6, 0]]),
            "c": jnp.array([[0, 0, 6], [0, 0, 8]]),
        },
    },
    {
        "contents": jnp.array([[1, 2, 3], [2, 3, 4]]),
        ...
        "expected": [
            jnp.array([[2, 0, 0], [4, 0, 0]]),
            jnp.array([[0, 4, 0], [0, 6, 0]]),
            jnp.array([[0, 0, 6], [0, 0, 8]]),
        ],
    },
    {
        "contents": jnp.array([[1, 2, 3], [2, 3, 4]]),
        "function": lambda x: Jaxtainer({"a": [x[0], {"c": x[1]}], "b": {"a": x[2]}})
        ** 2,
        "expected": {
            "a": [
                jnp.array([[2, 0, 0], [4, 0, 0]]),
                {"c": jnp.array([[0, 4, 0], [0, 6, 0]])},
            ],
            "b": {"a": jnp.array([[0, 0, 6], [0, 0, 8]])},
        },
    },
]
...
@pytest.mark.parametrize(
    ...
    [
        (test["contents"], test["function"], test["expected"])
        for test in jacfwd_and_vmap_constructor_tests
    ],
)
...
@pytest.mark.parametrize(
    "array, shape, function, expected",
    [
        (test["array"], test["shape"], test["function"], test["expected"])
        for test in from_array_tests
    ],
)
...
method_tests = [
    ...
    },
    {
        ...
        "expected": {
            "a": {"a": jnp.array([[[2]]]), "b": jnp.array([[[0]]])},
            "b": {"a": jnp.array([[[0]]]), "b": jnp.array([[[4]]])},
        },
    },
    {
        "contents": {"a": jnp.array([1]), "b": jnp.array([2])},
...
@pytest.mark.parametrize(
    "contents, function, expected",
    [(test["contents"], test["function"], test["expected"]) for test in method_tests],
)
